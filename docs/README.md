...
The file *env_make* is used to customize the *make* process for *AVR_C* development. It is **required** in order for make to properly identify the parameters needed for compiling/linking/uploading executable code to an AVR microcontroller **specific to the users computer**. 

**The file *env.def* is tracked by *git*. Use a copy command `cp env.def env.make` to create the env.make file on your system. Then make any required changes to the *env.make* file.** 
...
This page contains the files needed to be more efficient with *VS Code*. Install them inot the *.vscode* folder of *AVR_C*. The *.vscode* folder is now tracked by *git*.
